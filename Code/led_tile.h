...
//define three chars to keep track of linear representations of RGB values, prior to srgb conversion, to prevent the need to take sqrt.
char _r;
char _g;
char _b;
...
void init(void);
void set_lrgb(char r_new, char g_new, char b_new);
void set_rgb(char r_new, char g_new, char b_new);
void target_rgb(char r, char g, char  b, unsigned long time);
...
void target_rgb(char r, char g, char b, unsigned long time){
	...
	//time is expected to be a multiple of 500
	unsigned long interval = time/500;
	float r_change =  _r;
	r_change -= r;
	r_change /= interval;
...
void set_lrgb(char r_new, char g_new, char b_new){ //linear RGB color space - not recommended for use!
...
void set_rgb(char r_new, char g_new, char b_new){ //non-linear colorspace: instantly set output color
	//update internal linear values
	_r = r_new;
	_g = g_new;
	_b = b_new;
